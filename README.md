...
# [All the Mods 11 - ATM11](https://www.curseforge.com/minecraft/modpacks/all-the-mods-11) on Curseforge
<!-- toc -->
...
* JVM_OPTS "-Xms4G -Xmx8G" (the pack's default; ATM11 wants at least 8G for more than a couple players)
...
### Upgrading
When a new image version starts, it reinstalls the pack's `config`, `kubejs`, `local`, `mods` and NeoForge `libraries`. Your world, `server.properties`, whitelist and ops are kept. ATM11 is in beta, so back up `/data/world` before upgrading.

## Credits
Adapted from [docker-atmons](https://github.com/adamzaidi/docker-atmons).
